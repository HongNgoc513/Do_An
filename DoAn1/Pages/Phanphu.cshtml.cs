...
{
    public class PhanphuModel : PageModel
    {
        public DataTable PhanPhu { get; set; }
        ...
        public void TimKiem1(string query)
        {
            using (SqlConnection con = new SqlConnection(SQLConnect.Conn))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    ...
                    PhanPhu = new DataTable();
                    adapter.Fill(PhanPhu);
                }
            }
        }
        public void OnGet()
        {
            ...
            string query = @"SELECT * FROM SanPham INNER JOIN LoaiHang ON SanPham.Mahang = LoaiHang.Mahang WHERE Tenhang = N'Phan phu'";
            TimKiem1(query);
        }
    }
}
